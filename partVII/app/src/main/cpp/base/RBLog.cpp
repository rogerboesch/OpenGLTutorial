...
void rb_message_float1(const char* msg, float num1, int severity) {
    __android_log_print(severity == 1 ? ANDROID_LOG_ERROR : ANDROID_LOG_DEBUG, (const char*)"rblog", "%s: %f", msg, num1);
}
...
void rb_message_4d(const char* msg, float x1, float y1, float x2, float y2, int severity) {
    __android_log_print(severity == 1 ? ANDROID_LOG_ERROR : ANDROID_LOG_DEBUG, (const char*)"rblog", "%s: %f, %f| %f, %f", msg, x1, y1, x2, y2);
}
